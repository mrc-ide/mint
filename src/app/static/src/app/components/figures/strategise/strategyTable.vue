<template>
  <b-table :items="items">
    <template #cell(total_cases_averted)="data">
      <abbr :title="`${formatCases(data.value.median)} +${formatCases(data.value.maximum - data.value.median)} / -${formatCases(data.value.median - data.value.minimum)}`">
        {{ formatCases(data.value.median) }}
      </abbr>
    </template>
    <template #cell(percentage_of_total_cases_averted)="data">
      <abbr v-if="data.value" :title="`${formatPercentage(data.value.median)} +${formatCases((data.value.maximum - data.value.median) * 100, 1)} / -${formatCases((data.value.median - data.value.minimum) * 100, 1)}`">
    ...
    <template #cell(cost_per_case_averted)="data">
      <abbr v-if="data.value" :title="`${formatCost(data.value.median)} +${formatCost(data.value.maximum - data.value.median)} / -${formatCost(data.value.median - data.value.minimum)}`">
        {{ formatCost(data.value.median) }}
      </abbr>
    </template>
    <template #cell(cases_averted_per_person)="data">
      <abbr v-if="data.value" :title="`${formatCases(data.value.median, 1)} +${formatCases(data.value.maximum - data.value.median, 1)} / -${formatCases(data.value.median - data.value.minimum, 1)}`">
